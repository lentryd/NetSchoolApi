...
process.on("uncaughtException", (err) =>
  console.log("\x1b[30m\x1b[101m Error \x1b[0m", err)
);
...
export default class Parser {
  ...
  static checkHost(host: string): Promise<boolean> {
    return fetch(`http://${host}/webapi/prepareloginform`).then((res) => {
  ...
  static authForm(host: string): Promise<AuthForm> {
    ...
    return this.checkHost(host)
      .then((fit) => {
        if (!fit)
          throw new Error(
            `This server(${host}) is not suitable for the parser.`
          );
  ...
  static uploadAuthForm(
    host: string,
    allSelected: string
  ): Promise<AuthFormItems> {
    return this.checkHost(host)
      .then((fit) => {
        if (!fit)
          throw new Error(
          ...
          return fetch(
            `http://${host}/webapi/loginform?${allSelected}&LASTNAME=${
              allSelected.match(/(\w+?)=-*\d+?$/)[1]
            }`
          );
      })
      .then((res) => {
        ...
        return res
          ...
          .then((text) => {
            if (!text.includes("/asp/SecurityWarning.asp")) return res;
              ...
              return this.fetch("/asp/SecurityWarning.asp", {
                method: "post",
                body: `AT=${this._at}&WarnType=2`,
    ...
    return this.logOut()
      .then(() =>
        console.info(
          ...
          `The session for user '${this._login}' was successfully closed.`
        )
      )
      .catch((err) =>
        console.error(
          "\x1b[42m\x1b[30m Error \x1b[0m",
          `Failed to close session for user '${this._login}'.`,
          err
        )
      )
  ...
  private appContext(): Promise<void> {
    return this.fetch(`/asp/MySettings/MySettings.asp?at=${this._at}`, {
      ...
      .then((ctx) => {
        ...
        return this.fetch("/webapi/reports/studentgrades", {
          headers: {
            at: this._at,
          },
        });
      ...
      .then((data: Studentgrades) => {
        ...
        this.subjects = subjects.items.map((s) => ({
          id: s.value,
          name: s.title,
        }));
  ...
  public assignmentTypes(): Promise<AssignmentTypes[]> {
    return this.fetch("/webapi/grade/assignment/types").then((res) =>
      res.json()
    );
  }
